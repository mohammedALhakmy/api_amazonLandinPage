...
<template>
    ...
    <AuthenticatedLayout >
        ...
        <div class="max-w-[1000px] mx-auto flex gap-2 h-[270px]">
            <div class="border border-dotted border-gray-800 rounded-md w-1/3">
            ...
            <div v-if="$page.props?.auth?.address" class="relative border border-gray-400 rounded-md w-1/3 shadow-md">
                <div class="flex items-center justify-start p-3 text-xs text-gray-600 font-extrabold border-b border-b-gray-400">
                    Default : <img src="/images/logo/AMAZON_LOGO_DARK.png" alt="" class="h-3 mt-1.5 ml-2">
                </div>
                <div class="text-sm font-extrabold px-4 pt-4"> {{$page.props?.auth?.user?.first_name}} - {{$page.props?.auth?.user?.last_name}} </div>
                ...
                <div class="px-4 absolute bottom-0 pb-0">
                    <Link methods="delete" as="button" :href="route('address_options.destroy', $page.props?.auth?.address?.id)" class="text-teal-700 text-sm font-extrabold hover:underline hover:text-red-700">
                        Remove
                    </Link>
                </div>
            </div>
