    ...
    <AuthenticatedLayout>
         <div class="flex gap-4">
             <div class="w-full bg-white p-4 mt-4">
                 ...
                 <div v-if="!cart.length" class="text-center font-bold text-2xl py-20">Your Amazon Cart is Empty</div>
             ...
             <div class="bg-white w-[350px] p-4 mt-4 h-48">
                 <div class="text-sm text-green-700">
                     ...
                     Select This Delivery option at checkout.
                 </div>

                 <div class="font-extrabold text-[18px] pt-4">Subtotal (Items: {{cart.length }} ) : ${{total}}</div>
                 <Link class="block mt-4 w-full text-center py-1 font-bold text-sm rounded-lg border shadow-sm cursor-pointer"
                       as="button"
                       method="post"
                       :disabled="Number(total) === 0.00" :class="Number(total) === 0.00 ? 'bg-gray-400' : 'bg-yellow-400 hover:bg-yellow-500'"
                      :href="$page.props.auth.user !== null ?
                       route('checkout.store',{
                       total: totalWithoutDot(),
                       total_decimal: total,
                       items: cart
                       }) : route('login')">Proceed to Checkout</Link>
             </div>
         </div>
    </AuthenticatedLayout>
</template>
